...
module.exports = (it, expect, collect) => {
  ...
  it("should use null as value if value is missing", () => {
    const plucked = collection.pluck("manufacturer", "product");

    expect(plucked.all()).to.eql({
      Desk: "IKEA",
      Chair: "Herman Miller",
      Bookcase: "IKEA",
      Door: null,
  ...
  it("should allow wildcard dot notation", () => {
    ...
    expect(users.pluck("roles.*.name").all()).to.eql([["Editor", "Admin"]]);
  });
  ...
  it("should allow null as value in wildcard", () => {
    ...
    expect(users.pluck("roles.*.name").all()).to.eql([["Editor", null]]);
  });
  ...
  it("should allow undefined as value in wildcard", () => {
    ...
    expect(users.pluck("roles.*.name").all()).to.eql([["Editor", undefined]]);
  });
  ...
  it("should allow symbol as value in wildcard", () => {
    ...
    expect(users.pluck("roles.*.name").all()).to.eql([["Editor", symbol]]);
  });
  ...
  it("should allow multiple wildcards", () => {
    ...
    expect(users.pluck("*.*").all()).to.eql([[{ name: "Editor" }, { name: "Admin" }]]);

    expect(users.pluck("*.*.*").all()).to.eql([["Editor", "Admin"]]);
  });
};
